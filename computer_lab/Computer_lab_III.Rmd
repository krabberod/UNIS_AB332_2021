...
```{r}
identical(colnames(otu.tab.simple.gbm.clr),rownames(isa.metadata.simp))
```
...
```{r}
isa.metadata.cont<- isa.metadata.simp %>% dplyr::select("nitrate","phosphate","silicate","N_P",
                                                        "temp_25","sal_25","F_25",
                                                        "chla_GFF")
```
...
```{r}
corrplot(env.corr.signif.r , type="upper", order="hclust", 
         p.mat = env.corr.signif.p, sig.level = 0.05,insig = "pch", 
         hclust.method = c("average"), tl.cex= 0.8, tl.col="black", diag=F)
```
...
First we fit the varibles to the OTU-table that was rarified:  
**Rarefied table**
```{r}

otu.tab.simple.ss.nozero.bray.nmds.envfit<-envfit(otu.tab.simple.ss.nozero.bray.nmds,as.data.frame(isa.metadata.cont.zscores), permu=999,na.rm=T)
otu.tab.simple.ss.nozero.bray.nmds.envfit
...
```{r}
plot(otu.tab.simple.ss.nozero.bray.nmds, type="t", display="sites") # plot the samples
plot(otu.tab.simple.ss.nozero.bray.nmds.envfit) # plot all environmental vectors
```
The plotting only the vectors with p<0.01.
```{r}
plot(otu.tab.simple.ss.nozero.bray.nmds, type="t", display="sites") # plot the samples
plot(otu.tab.simple.ss.nozero.bray.nmds.envfit, p.max=0.01)
```
Can you see any difference?  

Now we do the same with the log-ratio transformed data:   
**clr table**

```{r}
otu.tab.simple.gbm.clr.euclidean.nmds.envfit<-envfit(otu.tab.simple.gbm.clr.euclidean.nmds,as.data.frame(isa.metadata.cont.zscores), permu=999, na.rm=T)
otu.tab.simple.gbm.clr.euclidean.nmds.envfit
```
...
```
Now as an exercise make the plot with only the vectors that have p<0.01 
```{r}
...
Lets use the **Rarefaction table** first:
Start with a model containing only species matrix and intercept:
```{r}
mod0.rarefaction<-capscale(otu.tab.simple.ss.nozero.bray~1, as.data.frame(isa.metadata.cont.zscores))
mod0.rarefaction
```

Now make a model including all variables from env matrix (the dot after tilde (~) means ALL!)
```{r}
mod1.rarefaction<-capscale(otu.tab.simple.ss.nozero.bray~ ., as.data.frame(isa.metadata.cont.zscores)) 
mod1.rarefaction
```
...
When doing a stepwise building of models you can this either "forward" (as in the example), "backwards", or "both". Try differnet methods and see if the endresult is any different. 


Here is the same stepwise selection for the **clr** data. Can you see any difference from the previous result? 
```{r}
#clr table
mod0.clr<-capscale(otu.tab.simple.gbm.clr.euclidean~1, as.data.frame(isa.metadata.cont.zscores)) # model containing only species matrix and intercept
mod1.clr<-capscale(otu.tab.simple.gbm.clr.euclidean~ ., as.data.frame(isa.metadata.cont.zscores)) # # model including all variables from env matrix (the dot after tilde (~) means ALL!)
...
**rarefied table**
```{r}
isa.rarified.db <- dbrda(formula = otu.tab.simple.ss.nozero.bray ~ silicate+temp_25+sal_25+F_25+phosphate, data = as.data.frame(isa.metadata.cont.zscores))
...
(isa.clr.dbrda<-dbrda(formula = otu.tab.simple.gbm.clr.euclidean ~ silicate+temp_25+sal_25+F_25+phosphate, data = as.data.frame(isa.metadata.cont.zscores)))
stats::screeplot(isa.clr.dbrda)
ggord(isa.clr.dbrda,xlims=c(-1.3,1.3), ylims=c(-1.5,1.5))
```

Ggord can take several parameters, for instance the seasons from the metadata:

```{r}
ggord(isa.clr.dbrda, isa.metadata.simp$seasons)
```
*See if you can modify the plot and make the text visible!*  
Or the months:

```{r}
ggord(isa.clr.dbrda, as.factor(isa.metadata.simp$month))
```
And you can specify the limits of the axes if the plot does not look nice:  
```{r}
ggord(isa.clr.dbrda, grp_in=as.factor(isa.metadata.simp$month), xlims=c(-1.3,1.3), ylims=c(-1.5,1.5))
...
**rarefaction table**
```{r}
mod0.cca.rarefaction<-cca(otu.tab.simple.ss.nozero~1, as.data.frame(isa.metadata.cont.zscores)) # model containing only species matrix and intercept
mod1.cca.rarefaction<-cca(otu.tab.simple.ss.nozero~ ., as.data.frame(isa.metadata.cont.zscores)) # # model including all variables from env matrix (the dot after tilde (~) means ALL!)
...
```{r}
isa.clr.cca<-cca(formula = otu.tab.simple.ss.nozero ~ chla_GFF+ silicate+ N_P, data = as.data.frame(isa.metadata.cont.zscores))
...
**rarefied table**
```{r}
adonis(otu.tab.simple.ss.nozero~ silicate+temp_25+sal_25+F_25+phosphate,
...
```{r}
adonis(as.data.frame(t(otu.tab.simple.gbm.clr))~silicate+temp_25+sal_25+F_25+phosphate,
       data = as.data.frame(isa.metadata.cont.zscores), 
